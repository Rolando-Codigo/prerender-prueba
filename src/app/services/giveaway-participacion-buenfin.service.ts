...
export class GiveawayParticipacionBuenFinService {
  ...
  public contador(recaptchaToken: string, host: string): Observable<GiveawayParticipacionAhorroContador> {
    return new Observable<GiveawayParticipacionAhorroContador>((observer) => {
      //
      const url = `${this._URL_CONTADOR}?fecha=${this.formatDate()}&promocion=${'participacion_buen_fin'}&captcha=${recaptchaToken}&host=${host}`;
